...
//        binding.txtNumberValue.setText(user.loggedIn?.)
   ...
   private fun getAccountData(slug:String){
        viewModel.getSettings(requireContext(),slug,object:AccountViewModel.ContentFetchListener{
            override fun onFetched(content: AccountResponse)
            {
                Log.d("ACCOUNT",Gson().toJson(content))
                //TODO Asher please add code for navigation to AccountDetailFragment and add two extra parameter and pass title and body
            }
        })
    }
}