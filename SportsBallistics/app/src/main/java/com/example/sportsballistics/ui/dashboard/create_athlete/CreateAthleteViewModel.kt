...
class CreateAthleteViewModel(application: Application) : AndroidViewModel(application)
{
    ...
    Will be used for athletes and trainer both
     */
    fun editAthlete(context: Context, userId: String, file: File?, name: String, address: String, state: String, zipcode: Int, city: String, status: String, contact_no: String, age: String, grade: String, password: String, package_type: String, club_name: String, role_id: String, email: String, mListener: ContentFetchListener)
    {
        mErrorListener.addDialog()
        val apiService = ApiClient.client(context).create(ApiInterface::class.java)
        val builder = MultipartBody.Builder().setType(MultipartBody.FORM)
